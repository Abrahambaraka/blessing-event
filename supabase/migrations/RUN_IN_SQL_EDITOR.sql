...
-- 010 — achat billets : incrementer sold sans etre super_admin
create or replace function public.be_increment_sold_counts(p_event_id text, p_items jsonb)
returns void language plpgsql security definer set search_path = public as $$
declare
  event_data jsonb; ticket_types jsonb; new_ticket_types jsonb := '[]'::jsonb;
  tt jsonb; tt_updated jsonb; item jsonb; tid text; add_qty int; current_sold int;
begin
  if p_items is null or jsonb_array_length(p_items) = 0 then return; end if;
  select data into event_data from public.be_events
  where id = p_event_id and status = 'published' for update;
  if event_data is null then raise exception 'Événement introuvable ou non publié'; end if;
  ticket_types := coalesce(event_data -> 'ticketTypes', '[]'::jsonb);
  for tt in select value from jsonb_array_elements(ticket_types) as t(value) loop
    tid := tt ->> 'id'; add_qty := 0;
    for item in select value from jsonb_array_elements(p_items) as i(value) loop
  ...
  update public.be_events set data = event_data, updated_at = timezone('utc', now()) where id = p_event_id;
end; $$;
...
-- 011 — corriger be_tickets_by_email (DISTINCT + ORDER BY)
create or replace function public.be_tickets_by_email(p_email text)
...
revoke all on function public.be_tickets_by_email(text) from public;
grant execute on function public.be_tickets_by_email(text) to anon, authenticated;
select public.be_tickets_by_email('test@example.com') as test_rpc;
