...
export function mapSupabaseUser(sbUser: SupabaseUser): User {
  return {
    id: sbUser.id,
    email: sbUser.email ?? '',
    name: (sbUser.user_metadata?.name as string) ?? sbUser.email?.split('@')[0] ?? 'Utilisateur',
    role: parseRole(sbUser.user_metadata?.role ?? sbUser.app_metadata?.role),
    phone: sbUser.user_metadata?.phone as string | undefined,
