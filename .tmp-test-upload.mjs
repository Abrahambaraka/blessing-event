...
// api/_lib/eventImagesStorage.ts
import { randomUUID } from "node:crypto";
var EVENT_IMAGES_BUCKET = "event-images";
async function ensureEventImagesBucket(admin) {
...
function getEventImagePublicUrl(admin, path) {
...
async function handler(req, res) {
  try {
    ...
    if (!requireRole(auth, ["super_admin"])) {
      return json(res, 403, { error: "Acc\xE8s admin requis." });
    }
    const body = req.body ?? {};
    ...
    if (!ALLOWED.has(String(contentType))) {
      return json(res, 400, { error: "Format non support\xE9 (JPG, PNG, WebP, GIF)." });
    }
    const buffer = Buffer.from(String(dataBase64), "base64");
    if (buffer.byteLength > MAX_BYTES) {
      return json(res, 400, { error: "Image trop volumineuse apr\xE8s compression (max ~1,5 Mo)." });
    }
    ...
    const message = err instanceof Error ? err.message : "Upload \xE9chou\xE9.";
